...
if not exist "%BACKUP_PATH%" (
    mkdir "%BACKUP_PATH%"
    echo تم إنشاء مجلد النسخ الاحتياطي في C:\backups
)

:: تحديد مسار pg_dump (الإصدار 18)
...
if %errorlevel% equ 0 (
    echo [تم] تم إنشاء النسخة: %BACKUP_PATH%\backup_%DATE%.backup
    del "%BACKUP_PATH%\error.log" 2>nul
) else (
    echo [خطأ] فشل إنشاء النسخة الاحتياطية
    echo راجع ملف الأخطاء: "%BACKUP_PATH%\error.log"
    if exist "%BACKUP_PATH%\error.log" type "%BACKUP_PATH%\error.log"
)
