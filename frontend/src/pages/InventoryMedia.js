...
import './InventoryGeneral.css'; // نفس التنسيق
...
const InventoryMedia = () => {
  ...
  const fetchItems = async () => {
    try {
      ...
      // تصفية المواد الإعلامية فقط
      const mediaItems = response.data.filter(item => item.category === 'media');
      setItems(mediaItems);
      setLoading(false);
    } catch (error) {
  ...
  return (
    <div className="inventory-general">
      <h2>المواد الإعلامية</h2>
      
      <div className="filters-toolbar">
        <SearchBar 
          onSearch={setSearchTerm} 
          placeholder="بحث في المواد الإعلامية..."
        />
