...
const Associations = () => {
  ...
  const fetchRecords = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/associations');
      setRecords(response.data);
      const total = response.data.reduce((sum, record) => sum + parseFloat(record.total_amount), 0);
      setTotalAmount(total);
    } catch (error) {
      console.error('خطأ في جلب سجلات الجمعية:', error);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post('http://localhost:5000/api/associations', {
  ...
  const handleUpdate = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.put(`http://localhost:5000/api/associations/${currentRecord.id}`, {
        date: formData.date.toISOString().split('T')[0],
        member_id: formData.member_id,
        total_amount: parseFloat(formData.total_amount),
        notes: formData.notes
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/associations/${id}`);
      fetchRecords();
    } catch (error) {
  ...
  return (
    <div className="associations">
      <div className="header">
        <h1><IoHandLeftOutline /> الجمعية</h1>
        <div>
